...
% inner product of differential of rbf and function F, adapted from rbf function!
% Y: KxN
function Y = drbf( X,C, F, type, eps, k )
type = lower(type);
...
for i = 1:size(Cbig,2)
    ...
    switch type
        case 'thinplate'
            yd = r.*(2*log(sqrt(r_squared))+1);  % + 0.5*sqrt(r_squared); % makes nonnegative, try to impose nonnegativity of K
           y = dot(yd, F);
            y(isnan(y)) = 0;
        case 'gauss'
